...
var encoder = (function(){
  ...
  function normalizeTypedValue (item, allowLiterals) {
    if (item === null || (allowLiterals && item === undefined)) {
      if (allowLiterals) {
        return {value: 'NULL', literal: true};
      }
      return null;
    }
    var value = null;
    ...
    if (item.hint) {
      value = item.value;
      if (value === null || (allowLiterals && value === undefined)) {
        if (allowLiterals) {
          return {value: 'NULL', literal: true};
        }
        return null;
      ...
      if (typeof type === 'string') {
        typeInfo = dataTypes.getByName(type);
      }
    ...
    if (!type) {
      ...
      if (allowLiterals && !type && value instanceof types.QueryLiteral) {
        return {value: value.toString(), literal: true};
      }
      if (!type) {
  ...
  function encode (item) {
    var typedValue = normalizeTypedValue(item);
    if (typedValue === null) {
      return null;
    }
    ...
    switch (type) {
      ...
      case dataTypes.list:
      case dataTypes.set:
        return encodeList(value, type, typedValue.subtype);
      case dataTypes.map:
        return encodeMap(value, typedValue.keyType, typedValue.valueType);
      default:
  ...
  function encodeMap(value, keyType, valueType) {
    var parts = [];
    var propCounter = 0;
    for (var key in value) {
      ...
      var valueBuffer = encode(mapValue);
      //include item byte length
  ...
  function stringifyValue (item) {
    var typedValue = normalizeTypedValue(item, true);
    var value = typedValue.value;
    if (typedValue.literal) {
      return value;
    }
    var type = typedValue.type;
    var subtype = typedValue.subtype;
    switch (type) {
      case dataTypes.int:
      case dataTypes.float:
      case dataTypes.double:
      ...
      case dataTypes.set:
        return stringifyArray(value, subtype, '{', '}');
      case dataTypes.map:
        return stringifyMap(value, typedValue.keyType, typedValue.valueType);
      default:
        throw new TypeError('Type not supported ' + type, value);
    }
  }
