...
Connection.prototype.open = function (callback) {
  ...
  if (!this.options.secureP) return this.open2(callback);

// tried to go the starttls path, but no luck, so we do it the hard way...
  portfinder.getPort({ port: 8897 }, function(err, portno) {
    var i, options, proxy;

    if (!!err) return callback(err);

    options = { host: self.options.host, port: self.options.tlsport, rejectUnauthorized: false, ca: [] };
    if ((!!self.options.keyPath) && (self.options.crtPath)) {
      options.key = fs.readFileSync(self.options.keyPath);
      options.cert = fs.readFileSync(self.options.crtPath);
    }
    if (!!self.options.ca) for (i = 0; i < self.options.ca.length; i++) options.ca[i] = fs.readFileSync(self.options.ca[i]);

    self.options.host = '127.0.0.1';
    self.options.port = portno;
    proxy = net.createServer({ allowHalfOpen: true }, function(socket) {
      var buffers, readyP, tunnel;
     
      buffers = [];
      readyP = false;
      socket.on('data', function(data) {
        if (readyP) return tunnel.write(data);
        buffers.push(data);
      }).on('error', function(err) {
        self.emit('log', 'error', 'local: ' + err.message);
        tunnel.destroy();
      }).on('close', function(errorP) {
        if (errorP) self.emit('log', 'error', 'local: unexpected close');
        tunnel.destroy();
      }).on('end', function() {
        self.emit('log', 'info', 'local: done');
      });
      
      tunnel = tls.connect(options, function() {
        var i;

        readyP = true;
        for (i = 0; i < buffers.length; i++) tunnel.write(buffers[i]);
      }).on('data', function(data) {
        try { socket.write(data); } catch(ex) { tunnel.end(); }
      }).on('error', function(err) {
        self.emit('log', 'error', 'remote: ' + err.message);
        socket.end();
      }).on('close', function(errorP) {
        if (errorP) self.emit('log', 'error', 'remote: unexpected close');
        socket.end();
      }).on('end', function() {
        self.emit('log', 'info', 'remote: done');
      });      

      proxy.close();
     }).on('clientError', function(err) {
       callback(err);
     }).listen(self.options.port, self.options.host, function() {
       self.open2(callback);
     });
  });
};

Connection.prototype.open2 = function (callback) {
  var self = this;
