...
var reList = /^(list|set)\s*<\s*(\w+)\s*>$/i;
var reMap = /^map\s*<\s*(\w+)\s*,\s*(\w+)/i;
...
var dataTypes = {
  ...
  getByName:  function(name) {
    ...
    var listMatches = reList.exec(typeInfo.name);
    if (listMatches) {
      typeInfo.name = listMatches[1].toLowerCase();
      typeInfo.subtype = listMatches[2];
    }
    else {
      var mapMatches = reMap.exec(typeInfo.name);
      if (mapMatches) {
        typeInfo.name = 'map';
        typeInfo.keyType = mapMatches[1];
        typeInfo.valueType = mapMatches[2];
      }
    }
